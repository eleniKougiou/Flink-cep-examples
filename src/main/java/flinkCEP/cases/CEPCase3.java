...
// Different after match skip strategies, looping pattern example
// Selected contiguity condition: Strict contiguity

public class CEPCase3 {
    ...
    public static void main (String[] args) throws Exception {
        ...
        AfterMatchSkipStrategy skipToFirst = AfterMatchSkipStrategy.skipToFirst("start"); // Skip "start" event (b)
        AfterMatchSkipStrategy skipToLast = AfterMatchSkipStrategy.skipToLast("start"); // Skip "start" event (b)
        ...
        }).oneOrMore().consecutive().next("next").where(new SimpleCondition<>() {
            @Override
            public boolean filter(Event value){
        ...
        env.execute("Flink CEP After Match Skip Strategies, Looping Pattern Example");
    }
}
